...
whether the same instance appears twice are irrelevant; two `Percentage(10)` values are interchangeable.
...
- **Q3 — which way does rounding go, and is it rounding of the discount or of the price?** Money
rounding is usually a policy or accounting decision, sometimes a legal one. *Assumed:* the discount is
rounded down, so the price rounds up and the shop keeps the fraction (B2). I would want confirmation,
and I would want to know whether rounding happens once per rule (as specified) or only once on the final
...
*Assumed:* no floor above 0 and no cap; all rules always apply. A "never more than 50% off any duck"
or "promotions are mutually exclusive, best one wins" policy is common enough that I would ask explicitly
