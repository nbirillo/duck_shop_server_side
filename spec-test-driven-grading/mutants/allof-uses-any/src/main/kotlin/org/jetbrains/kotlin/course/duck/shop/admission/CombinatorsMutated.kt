...
//
// Mutant allof-uses-any (must-kill)
//   Combinators.kt:  override fun admits(duck: Duck): Boolean = policies.all { it.admits(duck) }  ->  override fun admits(duck: Duck): Boolean = policies.any { it.admits(duck) }
//
// AllOf behaves like OR: one satisfied policy is enough.
// A suite that kills this mutant: a test where exactly one of several policies says no and AllOf must reject
