...
//
// Mutant anyof-uses-all (must-kill)
//   Combinators.kt:  override fun admits(duck: Duck): Boolean = policies.any { it.admits(duck) }  ->  override fun admits(duck: Duck): Boolean = policies.all { it.admits(duck) }
//
// AnyOf behaves like AND: every policy has to agree.
// A suite that kills this mutant: a test where exactly one of several policies says yes and AnyOf must admit
