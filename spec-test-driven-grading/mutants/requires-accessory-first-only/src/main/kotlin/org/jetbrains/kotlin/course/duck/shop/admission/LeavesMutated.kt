...
//
// Mutant requires-accessory-first-only (must-kill)
//   Leaves.kt:  override fun admits(duck: Duck): Boolean = duck.accessories.any { it.name == accessoryName }  ->  override fun admits(duck: Duck): Boolean = duck.accessories.firstOrNull()?.name == accessoryName
//
// RequiresAccessory only looks at the FIRST accessory instead of all of them.
// A suite that kills this mutant: a test where the matching accessory is not the first one in the list
...
class RequiresAccessory(private val accessoryName: String) : AdmissionPolicy {
    override fun admits(duck: Duck): Boolean = duck.accessories.firstOrNull()?.name == accessoryName
}
