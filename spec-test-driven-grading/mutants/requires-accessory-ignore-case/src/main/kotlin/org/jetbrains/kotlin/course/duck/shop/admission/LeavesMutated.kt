...
//
// Mutant requires-accessory-ignore-case (must-kill)
//   Leaves.kt:  override fun admits(duck: Duck): Boolean = duck.accessories.any { it.name == accessoryName }  ->  override fun admits(duck: Duck): Boolean = duck.accessories.any { it.name.equals(accessoryName, ignoreCase = true) }
//
// RequiresAccessory matches names case-insensitively.
// A suite that kills this mutant: a case-sensitivity test — "HAT" does not satisfy RequiresAccessory("hat")
...
class RequiresAccessory(private val accessoryName: String) : AdmissionPolicy {
    override fun admits(duck: Duck): Boolean = duck.accessories.any { it.name.equals(accessoryName, ignoreCase = true) }
}
