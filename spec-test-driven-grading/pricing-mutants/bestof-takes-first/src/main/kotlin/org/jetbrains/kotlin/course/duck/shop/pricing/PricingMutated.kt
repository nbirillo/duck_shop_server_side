...
//
// Mutant bestof-takes-first (must-kill)
//   Pricing.kt:  rule.options.minOfOrNull { apply(duck, it, price) } ?: price  ->  rule.options.firstOrNull()?.let { apply(duck, it, price) } ?: price
//
// BestOf offers the first option rather than the best one.
// A suite that kills this mutant: two options where the second is the better deal
...
private fun apply(duck: Duck, rule: DiscountRule, price: Int): Int = when (rule) {
    ...
    is DiscountRule.BestOf ->
        rule.options.firstOrNull()?.let { apply(duck, it, price) } ?: price
