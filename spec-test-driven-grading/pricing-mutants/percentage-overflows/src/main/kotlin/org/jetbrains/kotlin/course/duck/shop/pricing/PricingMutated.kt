...
//
// Mutant percentage-overflows (must-kill)
//   Pricing.kt:  discount = (price.toLong() * rule.percent / 100).toInt()  ->  discount = price * rule.percent / 100
//
// Multiplies in Int, so a large price overflows.
// A suite that kills this mutant: a duck priced near Int.MAX_VALUE
...
private fun apply(duck: Duck, rule: DiscountRule, price: Int): Int = when (rule) {
    is DiscountRule.Percentage -> reduce(price, discount = price * rule.percent / 100)
