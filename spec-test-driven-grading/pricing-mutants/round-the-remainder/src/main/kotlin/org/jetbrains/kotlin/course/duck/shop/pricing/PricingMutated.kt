...
//
// Mutant round-the-remainder (must-kill)
//   Pricing.kt:  discount = (price.toLong() * rule.percent / 100).toInt()  ->  discount = price - (price.toLong() * (100 - rule.percent) / 100).toInt()
//
// Keeps a rounded-down remainder instead of subtracting a rounded-down discount.
// A suite that kills this mutant: a price where the percentage is not exact: 95 at 10% is 86, not 85
...
private fun apply(duck: Duck, rule: DiscountRule, price: Int): Int = when (rule) {
    is DiscountRule.Percentage -> reduce(price, discount = price - (price.toLong() * (100 - rule.percent) / 100).toInt())
