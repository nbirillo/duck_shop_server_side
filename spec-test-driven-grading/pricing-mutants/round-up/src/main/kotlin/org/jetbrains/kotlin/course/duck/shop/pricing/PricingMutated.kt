...
//
// Mutant round-up (must-kill)
//   Pricing.kt:  discount = (price.toLong() * rule.percent / 100).toInt()  ->  discount = ((price.toLong() * rule.percent + 99) / 100).toInt()
//
// Rounds the discount up, so the rounding favours the customer.
// A suite that kills this mutant: any price where the percentage is not exact
...
private fun apply(duck: Duck, rule: DiscountRule, price: Int): Int = when (rule) {
    is DiscountRule.Percentage -> reduce(price, discount = ((price.toLong() * rule.percent + 99) / 100).toInt())
