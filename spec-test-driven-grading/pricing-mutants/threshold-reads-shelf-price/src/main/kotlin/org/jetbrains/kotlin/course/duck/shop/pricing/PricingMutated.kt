// GENERATED by `./gradlew generatePricingMutants` from pricing-mutants/catalog.json — do not edit by hand.
//
// Mutant threshold-reads-shelf-price (must-kill)
//   Pricing.kt:  if (price >= rule.threshold) reduce(price, discount = rule.amount) else price  ->  if (duck.price >= rule.threshold) reduce(price, discount = rule.amount) else price
//
// The bonus tests the shelf price rather than the price it is applied to, so an earlier discount can no longer push a duck under the threshold.
// A suite that kills this mutant: a percentage rule before the bonus that drops the price under the threshold
...
private fun apply(duck: Duck, rule: DiscountRule, price: Int): Int = when (rule) {
    ...
    is DiscountRule.BigSpenderBonus ->
        if (duck.price >= rule.threshold) reduce(price, discount = rule.amount) else price
