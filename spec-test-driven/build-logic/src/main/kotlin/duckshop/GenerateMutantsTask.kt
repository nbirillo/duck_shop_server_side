 ...
 * `mutantsAuthoredTests`, `mutantsOut`.
 */
abstract class GenerateMutantsTask @Inject constructor(
    ...
    fun run() {
        ...
        catalog.forEach { mutant ->
            ...
            moduleDir.resolve("build.gradle.kts")
                .writeText(buildScript(mutant, coreSrc, learnerTests, authoredTests))

            logger.lifecycle("[generateMutants] ${mutant.id} (${mutant.bucket}) — ${mutant.file}: ${mutant.what}")
        }
        ...
        baselineDir.resolve("build.gradle.kts")
            .writeText(buildScript(null, coreSrc, learnerTests, authoredTests))
    ...
    private fun header(mutant: Mutant, catalogPath: String): String = buildString {
        ...
        appendLine("// A suite that kills this mutant: ${mutant.hint}")
        appendLine()
    }
    ...
    private fun buildScript(
        ...
        //
        ${if (mutant != null) "// Mutant ${mutant.id} (${mutant.bucket}): ${mutant.what}" else "// BASELINE: no mutation at all."}
        //
        ${
            if (mutant != null) {
