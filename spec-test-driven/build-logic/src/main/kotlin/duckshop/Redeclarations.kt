 ...
 *
 * It never touches a function, and it only removes a type whose name really is declared in the types
 * source set. A helper type the agent invented stays, and so does every line of its logic. Both kinds
 * of removal are returned so the caller can print them and record them next to the module: **a rescue
 * that leaves no trace would let a broken output contract read as a clean run** — which is exactly how
 * the extraction-filename collision produced a fake 18/20.
 */
internal object Redeclarations {
     ...
     */
    internal fun strip(code: String, existing: Map<String, String>): Result {
        if (existing.isEmpty()) return Result(code, emptyList())
